...
class _ContainerListViewState extends State<ContainerListView> {
  ...
  Future<List<Data_Vehicle>> getDataVehicle() async {
    String path = "${Server.url}/vehicle/vehicle_controller.php";
    final response = await http.get(Uri.parse(path));
    if (response.statusCode == 200) {
      return Future<List<Data_Vehicle>>.value(convertToList(response));
    } else {
  ...
  void initState() {
    super.initState();
    getDataVehicle().then((value) => setState(() {
          data.clear();
          data.addAll(value);
        }));
  }
      ...
      itemBuilder: (context, index) {
        return Card(
          child: InkWell(
            child: ListTile(
              title: Text(
                data[index].model! + " " + data[index].color!,
